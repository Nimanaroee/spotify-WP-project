...
describe('ListenerManagementPage', () => {
  ...
  it('shows listener profile details and stream stats', () => {
    ...
    expect(screen.getByRole('tab', { name: /followers \(1\)/i })).toBeInTheDocument()
    expect(screen.getByText('Follower Friend')).toBeInTheDocument()
    expect(screen.getByRole('link', { name: /follower friend/i })).toHaveAttribute(
      'href',
      '/profile/follower_friend',
    )
    expect(screen.queryByRole('button', { name: /unfollow follower friend/i })).toBeNull()
  })
  ...
  it('hides profile photo upload for Basic subscribers', async () => {
    ...
    await user.click(screen.getByRole('button', { name: /edit/i, hidden: false }))

    expect(
      screen.queryByRole('button', { name: /change profile photo/i }),
    ).not.toBeInTheDocument()
    expect(screen.queryByLabelText(/profile photo upload/i)).not.toBeInTheDocument()
  })
  ...
  it('uploads and saves a profile photo for non-Basic subscribers', async () => {
    ...
    storage.set(
      ...
      users.map((storedUser) =>
        storedUser.id === 1 ? { ...storedUser, subscription_tier: 'silver' } : storedUser,
      ),
    )
    useAuthStore.setState({
      user: {
  ...
  it('saves edited personal information', async () => {
    ...
    expect(screen.getByRole('heading', { name: 'Updated Listener' })).toBeInTheDocument()
  })
  ...
  it('unfollows an account from the following list', async () => {
    ...
    await user.click(screen.getByRole('button', { name: /unfollow following friend/i }))

    expect(await screen.findByText('You unfollowed Following Friend.')).toBeInTheDocument()
    expect(screen.queryByText('Following Friend')).not.toBeInTheDocument()
  })
